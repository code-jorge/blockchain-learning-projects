...
export default function QuizQuestion({
  ...
  if (!address) {
    return <p className="text-red-800">Please connect your wallet before doing the vibe check!</p>
  }
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      const payload: CheckAnswerPayload = { questionIndex, answerIndex, message, signedMessage };
  ...
  const renderResult = () => {
      ...
      return (
        <>
          ...
          <p className='text-white'>
            ...
            and open it in the{" "}
            <Link href="/lounge">
              <a className="underline hover:no-underline text-yellow-400">lounge</a>
            </Link>
            !
          </p>
        </>
    ...
    if (answerResult === "incorrect") {
      return <p className="text-red-400">Sorry, that was incorrect!</p>;
    }
            ...
            <div className="space-y-4">
              {answers.map((answerText, i) => (
                <div key={i} className="flex items-center">
                  ...
                  <label
                    htmlFor={i.toString()}
                    className="ml-3 block cursor-pointer text-sm font-medium text-white peer-disabled:text-gray-500 peer-disabled:cursor-not-allowed capitalize"
                  >
                    {answerText}
