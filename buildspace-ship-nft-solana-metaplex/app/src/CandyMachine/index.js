...
const getCandyMachineState = async () => {
  ...
  const liveDate = new Date(goLiveData * 1000)
  const goLiveDateTimeString = `${liveDate.getDay()}/${liveDate.getMonth()+1}/${liveDate.getFullYear()}`;
...
const CandyMachine = ({ walletAddress }) => {
  ...
  useEffect(() => {
    getCandyMachineState().then(setCandyMachine);
  }, []);
  ...
  const mintToken = async () => {
    ...
    const userPayingAccountAddress = candyMachine.tokenMint
      ? (await getAtaForMint(candyMachine.tokenMint, walletAddress.publicKey))[0]
  ...
  return candyMachine && (
    <div className="machine-container">
      <DropTimer candyMachine={candyMachine} />
