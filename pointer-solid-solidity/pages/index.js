...
const Home = ()=> {
  ...
  const keyboardsContract = useMemo(()=> getKeyboardsContract(ethereum), [ethereum]);
  ...
  }, [connectedAccount, keyboardsContract]);

  const addContractEventHandlers = useCallback(() => {
    if (keyboardsContract && connectedAccount) {
  ...
  }, [connectedAccount, getKeyboards, keyboardsContract]);

  useEffect(getKeyboards, [getKeyboards])
  useEffect(addContractEventHandlers, [addContractEventHandlers]);
